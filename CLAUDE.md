...
Đây là knowledge base học tập cá nhân, vận hành theo mô hình mentor: mục tiêu là đi từ trình độ hiện tại lên senior bằng cách xây dựng các bộ tài liệu chuyên nghiệp (lý thuyết + ví dụ chạy được + bài tập + review) cho từng chủ đề kỹ thuật, bắt đầu từ số 0.
...
Repo còn có một bộ agent chuyên môn tại `.claude/agents/` (Software Architect, Backend/Database/Platform/Frontend Engineer, Security/Release/Quality Engineer, Instructional Designer, Technical Writer) — mỗi agent là một lens đánh giá/thiết kế riêng biệt, dùng được cả trong luồng `/learn` lẫn độc lập để review code bình thường, không phụ thuộc lệnh `/learn`. Các quy tắc dùng chung giữa nhiều agent nằm ở `.claude/standards/`.
